...
* **Mechanics:**
  ...
  4. If an unsupported plan or setup error is detected before response transmission begins, the router selects Legacy V1. Once V2 has sent response headers or body bytes, it cannot transparently fall back; it must report the streaming error according to the server's existing connection/error policy.

### 3. Performance Rationale
* **Low Routing Overhead on Normal Queries:** Eligibility is evaluated during query plan translation by inspecting the parsed plan and supported operators; the implementation should benchmark this overhead.
...
* **Mechanics:**
  ...
  2. Instead of copying strings into a formatted buffer, the serializer creates a bounded batch of `struct iovec` descriptors containing:
     ...
     The batch has an explicit maximum descriptor count and byte budget. When either limit is reached, the batch is submitted before more rows are serialized; the streaming abstraction retains the arena storage, iovec descriptors, and owning slot until the entire send completes. After a partial write, it advances the iovec base pointers and lengths and retains the remaining descriptors and backing storage; serialization yields until completion or writability is reported, so in-flight descriptors and backing storage remain bounded. A slot is recycled or mutated only after the final write completion or corresponding io_uring CQE.
