...
* **Mechanics:**
  ...
  4. If an unexpected runtime condition arises before the HTTP response is committed, the router falls back to Legacy V1. Failures detected after response bytes have been sent must be reported as an interrupted V2 response.
...
### 3. Performance Rationale
* **Intermediate Buffer Traffic Reduction:** V2 avoids materializing the complete result as an `IdTable` and processes fixed-size chunks, reducing intermediate writes and reads. The source data and chunks may still access DRAM when they do not fit in cache.
...
### 3. Performance Rationale
* **Reduced Intermediate Copies:** The serializer avoids copying each term into a contiguous application-side payload buffer. Kernel copying may still occur for ordinary `writev`; zero-copy transmission requires the relevant socket and buffer-lifetime support.
...
* **Validation plan:** Verify behavior across 128B to 4096B literal sizes and require a successful test exit status.
