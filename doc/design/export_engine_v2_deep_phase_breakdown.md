...
* **Mechanics:**
  ...
  4. Fallback to Legacy V1 is permitted only before the response headers are sent. After headers or body transmission begins, any V2 failure is a terminal stream error reported through the active response path; route selection does not itself imply recoverability.

### 3. Performance Rationale
* **Low Overhead on Normal Queries:** Evaluation checks request metadata and the ParsedQuery execution plan during query plan translation; its cost depends on the plan size and supported operators.
...
* **Assertion:** Verify that V2 setup failures before response headers are sent select V1 without emitting an HTTP error, while failures after headers or body bytes are sent do not fall back to V1 and instead produce the defined terminal streaming error through the active response path.
...
* **Mechanics:**
  ...
  5. Buffer ownership is part of each in-flight transmission: construction pairs the `struct iovec` entries with owning storage for the vocabulary arena, static delimiter storage, HTTP framing bytes, and every other referenced buffer. The transmission is finalized only by consuming the assembled transmission state, which transfers the complete ownership bundle to the asynchronous send operation. That bundle remains alive and unchanged until `writev` has returned or the asynchronous completion event has been observed; only then may its buffers be reset or reused.
