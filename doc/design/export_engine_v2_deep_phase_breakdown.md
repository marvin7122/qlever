...
* **Mechanics:**
  1. Centralizes mode selection in one precedence-ordered decision function: first the explicit request parameter (`fast-export=1` or `export-engine=v2`), then the HTTP header (`X-QLever-Export-Engine: v2`), and finally the server default.
  ...
  4. If an unexpected runtime condition arises during V2 planning or setup, before any output bytes are committed, it triggers a clean fallback to Legacy V1. Once the first byte is committed, fallback is unavailable; any subsequent V2 failure is reported as an explicit stream failure rather than switching to Legacy V1.

### 3. Performance Rationale
* **Low Overhead on Normal Queries:** Request-option parsing is constant-time; plan eligibility inspection traverses the relevant parsed-plan operators and should be measured independently.
...
### 3. Performance Rationale
* **Reduced Intermediate-Table Traffic:** In Legacy V1, a 10,000,000-row export writes and reads 240 MB of `IdTable` buffers to/from main DRAM. V2 avoids materializing the complete result table; actual cache and DRAM traffic depends on vocabulary accesses, cache eviction, and output-buffer writes.
...
* **Mechanics:**
  ...
  4. The transmission owns the arena allocation and every referenced buffer until completion. For synchronous `::writev()`, the arena and `iovec` storage remain valid until the call returns; for `io_uring_prep_send_zc`, they remain valid until the corresponding completion notification, including any zero-copy notification required by the socket path. The in-flight transmission releases the allocation only after its API-specific lifetime requirement has been satisfied.

### 3. Performance Rationale
* **Fewer Intermediate Copies:** The serializer avoids copying each term into an intermediate concatenation buffer; escaping, framing, and the selected socket API may still require additional copies.
...
### 3. Performance Rationale
* **Compute/I/O Overlap:** Asynchronous socket operations allow the worker to generate the next chunk while a previous write is in progress, when the API and socket buffer permit it.
* **Latency Hiding:** This can reduce stalls caused by network backpressure; the amount of latency hidden depends on chunk size, CPU cost, and socket behavior.
