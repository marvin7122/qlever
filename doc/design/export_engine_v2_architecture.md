...
3. **Automatic Safe Fallback:**
   - If an export query contains unsupported complex operators (e.g. distributed aggregation), the router executes the query with Legacy V1 instead of returning an unsupported-query error.
...
4. **Single-Core Discipline:** The query worker interleaves computation with non-blocking socket writes; no additional query thread is used.
