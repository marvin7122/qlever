...
// fills a caller-provided `ArenaVocabBatchBuilder`. Written with `void_t`
// instead of `if constexpr (requires { ... })`, which is C++20-only and
// therefore not available in the C++17 CI configurations.
template <typename Vocab, typename = void>
...
// `PolymorphicVocabulary`) use this to fill the caller builder when the
// overload exists and fall back to copying single-shot words otherwise.
template <typename Vocab>
