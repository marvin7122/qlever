...
// Allocate a string of `bound` bytes, decode directly into its buffer via the
// `decode` invocable (which receives a `ql::span<char>` of that size and
// returns the number of bytes actually written), and resize the string to that
// decoded size. The buffer is not zero-initialized when
