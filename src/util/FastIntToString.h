...
inline void format8Digits(uint32_t v, char* dst) noexcept {
  ...
  // tens = (pairs * 52429) >> 19  (approx division by 10). 52429 exceeds
  // INT16_MAX, but its bit pattern is exactly what the unsigned multiply-high
  // needs, so the narrowing is made explicit.
