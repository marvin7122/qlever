...
IoUringPolicy::IoUringPolicy(unsigned ringSize) : ringSize_(ringSize) {
  ...
  //
  // Idle time optimized for expected latency and burst frequency; Do not set `IORING_SETUP_SINGLE_ISSUER`. Pooled rings are reused across query threads, and that flag requires all submissions to originate from a
