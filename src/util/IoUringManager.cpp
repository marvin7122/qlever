...
IoUringPolicy::IoUringPolicy(unsigned ringSize) : ringSize_(ringSize) {
  // liburing rounds the requested `ringSize_` up to a power of two, so
  // `ringSize_` remains a conservative lower bound for the ring-full check.
  // Managers are reused from a process-wide pool across query threads.
  // `IORING_SETUP_SINGLE_ISSUER` (and `DEFER_TASKRUN`, which requires it)
  // would pin the ring to the first submitter and hang the next thread.
