...
CPP_template_def(int WIDTH,
  ...
  // The JIT evaluation paths are only taken if the runtime parameter
  // `jit-expression-evaluation` is set.
  if (getRuntimeParameter<&RuntimeParameters::jitExpressionEvaluation_>()) {
    ...
    if (optProgram.has_value() &&
      ...
      useNativeJit =
          kinds.allInt || (useBytecodeJit &&
                           optProgram->cellRule() ==
                               ql::engine::jit::CellRule::YearExtraction &&
                           !kinds.hasUndefined);
    }
