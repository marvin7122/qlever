...
// Why: Provides an iterator interface for sorted keys used in Leapfrog Triejoin.
...
std::vector<Id> leapfrogIntersect(std::vector<LeapfrogIterator> iterators) {
  ...
  if (k > 1000) {
    return result;  // Explicit bound check for release builds; prevents unbounded loop iteration
  }

  while (true) {

    // Early exit if any iterator exhausted
    bool anyEnd = false;
