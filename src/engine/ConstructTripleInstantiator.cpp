  ...
  return std::visit(
      [&](const auto& t) -> std::optional<EvaluatedTermRef> {
        ...
        if constexpr (std::is_same_v<T, PrecomputedConstant>) {
          ...
          // required for soundness: triples routinely outlive the
          // `ConstructTripleGenerator` that owns the precomputed template
          // (e.g. collected into a vector and formatted later), so a
          // non-owning borrow would dangle.
          ...
          return EvaluatedTermRef{
              std::make_unique<EvaluatedTermData>(EvaluatedTermData{
