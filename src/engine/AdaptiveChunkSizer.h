...
class AdaptiveChunkSizer {
 ...
 public:
  ...
  [[nodiscard]] size_t targetRowCount() const noexcept {
    ...
    // Clamp before the float-to-`size_t` conversion: `ceil` of a value past
    // `SIZE_MAX` (reachable with adversarial configs) is undefined behavior,
    // and anything at or above `maxChunkRows_` clamps to it anyway.
    if (!(rawTargetRows < static_cast<double>(config_.maxChunkRows_))) {
      return config_.maxChunkRows_;
    }
  ...
  void recordChunk(size_t bytesWritten, size_t rowCount) {
    ...
    // Exponential ramp-up towards maxChunkBytes_. The bound check precedes
    // the conversion: with an adversarial `growthFactor_`, `ceil` can exceed
    // `SIZE_MAX`, which makes the cast undefined behavior.
    if (currentChunkBytesTarget_ < config_.maxChunkBytes_) {
      ...
      if (!(nextBytes < static_cast<double>(config_.maxChunkBytes_))) {
        currentChunkBytesTarget_ = config_.maxChunkBytes_;
      } else {
        currentChunkBytesTarget_ = static_cast<size_t>(std::ceil(nextBytes));
      }
    }
  }
