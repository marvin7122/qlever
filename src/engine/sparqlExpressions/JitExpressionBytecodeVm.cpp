...
std::optional<JitBytecodeProgram> JitExpressionBytecodeVm::compile(
  ...
  if (expr.compileToJit(program, varColMap)) {
    // Right-nested expressions (e.g. `1 + (1 + (...))`) grow the stack
    // linearly, and the kernels use fixed `MAX_STACK_SLOTS` stacks. Overly
    // deep programs fall back to the generic evaluation.
    if (!program.fitsInterpreterStack()) {
      return std::nullopt;
    }
