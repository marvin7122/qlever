...
// _____________________________________________________________________________
// Name helpers for the currently running gtest, with every '/' replaced by
// '_' (parameterized tests embed '/' in their names). `sanitizeGtestName`
// performs the replacement on a given raw name; it is the shared
// implementation behind `gtestCurrentTestName` and
// `gtestCurrentTestSuiteName`. `gtestCurrentTestName` returns
// "<TestSuiteName>_<TestName>" for the currently running gtest. If
// `assertInGtestEnvironment` is true (the default), it crashes when called
