...
// `finalize` exposes the span over the filled views, and the returned owning
// result keeps the backing buffer/views alive after the builder is destroyed
...
// Tests for `ArenaVocabBatchBuilder`: the `monotonic_buffer_resource` backing
...
TEST(ArenaVocabBatchBuilder, AppendedWordsStayValidInFinalizedResult) {
