...
// in an export response. The batch arms copy the looked-up views to result
// slots in input order, as production `idsToStringAndType` scatters each
// resolved value back into its slot (partitioned by datatype, no sorting).
// The mock multiset holds only `VocabIndex` IDs, so the production datatype
// partition is a no-op here. The vocabulary is synthetic, so only the
// relative overhead (allocation, dispatch, escaping) transfers; absolute
// times need the full export A/B on Wikidata truthy with the byte-identity
// gate.
...
class VocabBatchExportMock : public BenchmarkInterface {
  ...
  // Arm 2: batched lookup, fresh string per cell for escaping. The views
  // are copied to result slots in input order, as production
  // `idsToStringAndType` scatters each resolved value back into its slot.
  void runBatch() {
    chunk_.clear();
    ...
    for (size_t begin = 0; begin < multiset_.size(); begin += batchSize) {
      ...
      rowViews.assign(result->begin(), result->end());
