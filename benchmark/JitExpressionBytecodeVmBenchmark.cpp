...
class JitExpressionBytecodeVmBenchmark : public BenchmarkInterface {
 public:
  std::string name() const final {
    return "JIT Expression Bytecode VM Benchmark";
  }
  ...
  BenchmarkResults runAllBenchmarks() final {
    ...
    results.addMeasurement("Virtual AST Tree: (col0 * 2) + col1 > 100",
                           [&]() { return astMatches; });
    ...
    results.addMeasurement("JIT Bytecode VM: (col0 * 2) + col1 > 100",
                           [&]() { return jitMatches; });
    ...
    results.addMeasurement("Native AsmJit: (col0 * 2) + col1 > 100",
    ...
    generalMeta.addKeyValuePair("bytecode_ms", jitMs);
    generalMeta.addKeyValuePair("native_ms", asmMs);
    generalMeta.addKeyValuePair("compile_us", compileUs);
    generalMeta.addKeyValuePair("speedup_bytecode_vs_ast", astMs / jitMs);
    generalMeta.addKeyValuePair("speedup_native_vs_ast", astMs / asmMs);
    generalMeta.addKeyValuePair("speedup_native_vs_bytecode", jitMs / asmMs);

    return results;
  }
};

AD_REGISTER_BENCHMARK(JitExpressionBytecodeVmBenchmark);
