...
// Compare `std::to_string`, `std::to_chars`, and `formatIntBranchless`.
 ...
 public:
  ...
  BenchmarkResults runAllBenchmarks() final {
    ...
    // Benchmark sequential 64-bit integer formatting.
    {
      ...
      // Benchmark `std::to_string` as the baseline with dynamic allocation and standard division.
      {
        size_t totalBytes = 0;
    ...
    // Benchmark Wikidata entity IRI formatting using `"http://www.wikidata.org/entity/Q"` and an ID.
    {
      ...
      // 2. std::to_chars with manual prefix copy
      {
        ...
        auto& m = group.addMeasurement("memcpy prefix + std::to_chars", [&]() {
          size_t bytes = 0;
          for (uint64_t id : qids) {
            constexpr std::string_view prefix = "http://www.wikidata.org/entity/Q";
            std::memcpy(buffer, prefix.data(), prefix.size());
            auto [ptr, ec] =
                std::to_chars(buffer + prefix.size(), buffer + sizeof(buffer), id);
            bytes += static_cast<size_t>(ptr - buffer);
          }
          totalBytes = bytes;
          return bytes;
