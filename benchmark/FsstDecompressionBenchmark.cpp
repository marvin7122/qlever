...
class FsstDecompressionBenchmark : public BenchmarkInterface {
 ...
 public:
  FsstDecompressionBenchmark() {
    ...
    ad_utility::RandomCustomStringGenerator gen{
        "abcdefghijklmnopqrstuvwxyz0123456789_:/.-#"};
    for (size_t i = 0; i < 5000; ++i) {
      shortWords_.push_back("http://example.org/p/" + gen(15));
      mediumWords_.push_back("http://www.wikidata.org/entity/Q" + gen(45));
      longWords_.push_back(
          "http://en.wikipedia.org/wiki/Special_Entity_Resource_Descriptor_" +
          gen(180));
    }

    auto setupDecoders = [](const std::vector<std::string>& words,
                            auto& decoderOpt, auto& compressedViews,
                            auto& storageBuffers) {
      constexpr size_t N =
          std::decay_t<decltype(*decoderOpt)>::Decoders{}.size();
  ...
  BenchmarkResults run() const override {
    ...
    auto& group = results.addResultGroup(
        "FSST Repeated Decompression Benchmarks (5,000 words)");
    ...
    group.addMeasurement("N=1: decompressInto() [direct zero-scratch]", [&]() {
      ...
      for (const auto& c : compressed1_) {
        size_t n = decoder1_->decompressInto(
            c, ql::span<char>{outBuf.data(), outBuf.size()});
        totalBytes += n;
      }
      return totalBytes;
    });
    ...
    group.addMeasurement(
        "N=2: decompressInto() [reused persistent scratch]", [&]() {
          size_t totalBytes = 0;
          std::string scratch;
          std::string outBuf(FsstRepeatedDecoder<2>::maxDecompressedSize(
                                 std::string_view(mediumWords_.front())),
                             '\0');
          for (const auto& c : compressed2_) {
            size_t n = decoder2_->decompressInto(
                c, ql::span<char>{outBuf.data(), outBuf.size()}, scratch);
            totalBytes += n;
          }
          return totalBytes;
        });
    ...
    group.addMeasurement(
        "N=3: decompressInto() [reused persistent scratch]", [&]() {
          size_t totalBytes = 0;
          std::string scratch;
          std::string outBuf(FsstRepeatedDecoder<3>::maxDecompressedSize(
                                 std::string_view(mediumWords_.front())),
                             '\0');
          for (const auto& c : compressed3_) {
            size_t n = decoder3_->decompressInto(
                c, ql::span<char>{outBuf.data(), outBuf.size()}, scratch);
            totalBytes += n;
          }
          return totalBytes;
