...
int main() {
  ...
  std::cout
      << "=================================================================\n";
  std::cout << "Comparative Benchmark: Exact std::unordered_set vs "
               "HyperLogLog++ Sketch\n";
  std::cout << NUM_ITEMS << " stream items (" << DISTINCT_ITEMS
            << " distinct)\n";
  std::cout
      << "=================================================================\n";
  ...
  size_t exactMemoryBytes =
      exactSet.bucket_count() * sizeof(void*) +
      exactSet.size() * (sizeof(uint64_t) + sizeof(void*));
  ...
  double relError = std::abs(static_cast<double>(hllEstimate) -
                             static_cast<double>(DISTINCT_ITEMS)) /
                    static_cast<double>(DISTINCT_ITEMS) * 100.0;

  std::cout << "\n--- Baseline: Exact std::unordered_set ---\n";
  std::cout << "Runtime: " << exactMs << " ms ("
            << (NUM_ITEMS / (exactMs / 1000.0)) / 1e6
            << " M items/sec, distinct: " << exactCount << ")\n";
  std::cout << "Memory:  " << exactMemoryBytes / (1024 * 1024) << " MB\n";

  std::cout << "\n--- Prototype: HyperLogLog++ Sketch ---\n";
  std::cout << "Runtime: " << hllMs << " ms ("
            << (NUM_ITEMS / (hllMs / 1000.0)) / 1e6
            << " M items/sec, estimate: " << hllEstimate
            << ", error: " << relError << "%)\n";
  std::cout << "Memory:  " << hllMemoryBytes / 1024 << " KB\n";

  std::cout << "\n============================================================="
               "====\n";
  std::cout << ">>> HLL Throughput Speedup: " << (exactMs / hllMs)
            << "x faster\n";
  std::cout << ">>> Memory Reduction:       "
            << (exactMemoryBytes / hllMemoryBytes) << "x smaller footprint\n";
  std::cout
      << "=================================================================\n";
